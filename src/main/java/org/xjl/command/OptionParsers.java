...
class OptionParsers {
    ...
    public static OptionParser<Boolean> bool() {
        return (arguments, option) -> getValues(arguments, option, 0)
    ...
    public static <T> OptionParser<T[]> list(IntFunction<T[]> generator, Function<String, T> valueParser) {
        return (arguments, option) -> getValues(arguments, option)
                .map(it -> it.stream().map(value -> parseValue(option, value, valueParser)).toArray(generator)).orElse(generator.apply(0));
    }

    public static Optional<List<String>> getValues(List<String> arguments, Option option) {
        int index = arguments.indexOf("-" + option.value());
        return Optional.ofNullable(index == -1 ? null : getValues(arguments, index));
    }

    public static List<String> getValues(List<String> arguments, int index) {
        return arguments.subList(index + 1, IntStream.range(index + 1, arguments.size())
                .filter(it -> arguments.get(it).matches("^-[A-Za-z]+$")).findFirst().orElse(arguments.size()));
    }

    public static Optional<List<String>> getValues(List<String> arguments, Option option, int expectedSize) {
